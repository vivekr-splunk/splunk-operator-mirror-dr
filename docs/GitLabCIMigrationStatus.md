...
- The standard build workflow class now includes:
  ...
  - the Trivy installer now resolves the exact `Linux-64bit` release asset through the GitHub releases API and defaults to the approved safe pin `v0.69.3`, while still supporting an explicit `STAGING_TRIVY_RELEASE` override for controlled future upgrades; this replaced an earlier hard-coded `v0.69.4` reference that did not exist upstream
